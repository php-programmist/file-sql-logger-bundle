...
namespace PhpProgrammist\FileSqlLoggerBundle;
...
class FileSqlLogger implements SQLLogger
{
    ...
    public function __construct(ParameterBagInterface $params,Filesystem $filesystem,?string $pathToLogs=null)
    {
        ...
        if (null === $pathToLogs) {
            $pathToLogs = $params->get('kernel.project_dir').'/sql/';
        }
        $this->filename = $pathToLogs.'log_'.$params->get('kernel.environment').'.sql';
    }
